...
class QueryableTest(TestCase):
    ...
    def test_sql(self):
        sql = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).sql
    ...
    def test_count(self):
        count = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).count()
        self.assertEquals(
            count,
            2,
            u"Number of students inserted should equal 2 - get {0}".format(count))
    ...
    def test_take(self):
        ...
        self.assertEquals(
            len(result), 1, u"Appears that take expression is not working")
        self.assertEquals(
            result[0].student_id,
            1,
        ...
        self.assertEquals(
            result[0].first_name,
            u"Bruce", u"Bruce should be the first name - get {0}".format(
                result[0].first_name)
        )
        self.assertEquals(
            result[0].last_name,
            u"Fenske", u"Fenske should be the last name - get {0}".format(
                result[0].last_name)
        )
        ...
        self.assertEquals(
            result[0].first_name,
            u"Abraham", u"Abraham should be the first name - get {0}".format(
                result[0].first_name)
        )
        self.assertEquals(
            result[0].last_name,
            u"Mudryk", u"Mudryk should be the last name - get {0}".format(
                result[0].last_name)
        )

    def test_skip(self):
        result = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).skip(1).to_list()
        self.assertEquals(
            len(result),
            1,
        ...
        result = self.conn.query(
            operators.SelectOperator(
                ...
                lambda x: x.first_name)) .take(1).skip(1).to_list()
        ...
        result2 = self.conn.query(
            operators.SelectOperator(
                expressions.TableExpression(Student),
                lambda x: x.first_name)) .skip(1).take(1).to_list()
    ...
    def test_first(self):
        result = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).first()
        self.assertEquals(
            result.student_id,
            1,
        ...
        self.assertRaises(
            NoElementsError,
            self.conn.query(operators.SelectOperator(
                expressions.TableExpression(Student))).first
        )
    ...
    def test_first_or_default(self):
        ...
        result = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).first_or_default()
        self.assertEquals(result.student_id, 2,
                          u"Student ID should be 2 - get {0}".format(result.student_id))
        self.assertEquals(
            result.first_name,
            u"Abraham",
        ...
        self.assertIsNone(
            result,
            u"First or Default query should be none. The Student table is empty")

    def test_where(self):
        students = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).where(lambda s: s.student_id == 1)
        num_students = students.count()
        self.assertEquals(
            num_students, 1, u"Should only be one student row returned in query")
        ...
        students = self.conn.query(operators.SelectOperator(expressions.TableExpression(
            Student))).where(lambda s: s.student_id == 1 or s.gpa >= 50)
        self.assertEquals(students.count(), 2)

        students = self.conn.query(operators.SelectOperator(expressions.TableExpression(
            Student))).where(lambda s: s.student_id == 1 and s.gpa > 50)
    ...
    def test_single(self):
        ...
        self.assertRaises(MoreThanOneMatchingElement,
                          students.single, lambda s: s.gpa > 0)
    ...
    def test_single_or_default(self):
        ...
        self.assertIsNone(students.single_or_default(
            lambda s: s.first_name == u"Wayne" and s.last_name == u"Gretzky"))
        self.assertRaises(MoreThanOneMatchingElement,
                          students.single_or_default, lambda s: s.gpa > 0)
    ...
    def test_order_by(self):
        ...
        for s in ordered_students:
            self.assertTrue(highest_grade <= s.gpa,
                            u"{0} is not <= {1}".format(highest_grade, s.gpa))
            highest_grade = s.gpa

        # Try ordering multiple columns
        ordered_students = students.order_by(
            lambda x: x.last_name).then_by(lambda x: x.first_name).to_list()
    ...
    def test_order_by_multiple(self):
        ...
        # Try ordering multiple columns
        ordered_students = students.order_by(
            lambda x: x.gpa).then_by(lambda x: x.last_name).to_list()
    ...
    def test_max(self):
        max_gpa = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).max(lambda s: s.gpa)
        self.assertEquals(max_gpa, 50)

    def test_min(self):
        min_gpa = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).min(lambda s: s.gpa)
        self.assertEquals(min_gpa, 9)

    def test_sum(self):
        sum_gpa = self.conn.query(operators.SelectOperator(
            expressions.TableExpression(Student))).sum(lambda s: s.gpa)
